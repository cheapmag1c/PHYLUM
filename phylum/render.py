...
# === VIVARIUM + CONVERGENCE RENDER LAYER v2.0.1 START ===
from .vivarium_render import render_vivarium_assets as _render_vivarium_assets


def _update_readme_convergence(world, root: Path) -> None:
    """Make ORRERY the single public observatory and VIVARIUM the engine beneath it."""
    readme = root / "README.md"
    if not readme.exists():
        return
    import re as _conv_re
    ...
    # Remove the old second hero. VIVARIUM remains an engine, not a competing UI.
    text = _conv_re.sub(
    ...
    living = (
        ...
        f"Continuous time: **day {sim_day:.0f} / year {sim_year:.2f}** at observation **{obs}**. "
        "Open [`docs/life.html`](docs/life.html) for the ORRERY **LIFE** view.\n\n"
        "The historical `docs/vivarium.html` URL now redirects to LIFE so there is only one observatory hierarchy.\n"
    )
    text = _conv_re.sub(r"\n## Living engine — VIVARIUM\n.*?(?=\n## Observatory|\n## License)", "\n", text, flags=_conv_re.S)
    marker = "\n## Observatory\n"
    if marker in text:
        text = text.replace(marker, "\n" + living + marker, 1)
    else:
        marker = "\n## License\n"
        text = text.replace(marker, "\n" + living + marker, 1) if marker in text else text + "\n\n" + living

    # Normalize VIVARIUM's checkpoint vocabulary after older render layers have
    # regenerated their legacy generation-era prose.
    ...
    if "**Simulated time:**" in text:
        text = _conv_re.sub(r"\*\*Simulated time:\*\* `[^`]+` / `[^`]+`  \n", f"**Simulated time:** `day {sim_day:.0f}` / `year {sim_year:.2f}`  \n", text, count=1)
    else:
        text = _conv_re.sub(r"(\*\*Observation:\*\* `[^`]+`  \n)", rf"\1**Simulated time:** `day {sim_day:.0f}` / `year {sim_year:.2f}`  \n", text, count=1)
    text = text.replace(
    ...
    text = text.replace("current-generation events", "current-checkpoint events")
    ...
    # Normalize 2.0 vocabulary and file hierarchy.
    ...
    text = text.replace("vivarium_render.py          # living-world observation surface", "vivarium_render.py          # ORRERY LIFE view renderer")
    text = text.replace("vivarium.svg                # VIVARIUM individual/cohort world view", "life.svg                    # ORRERY LIFE / VIVARIUM engine view")
    text = text.replace("Open `docs/vivarium.html` or `renders/vivarium.svg` for the current individual/cohort engine state.\n", "")

    # Explicit conceptual hierarchy, once.
    hierarchy = (
        "\n### System hierarchy\n\n"
        "```text\n"
...
def render_all(world, species, env, pathogens, plates, branch, interactions):
    # All legacy biological/planetary/social renderers still run first. ORRERY
    # remains their final composite shell; LIFE is then rendered as a sub-view.
    ...
    _update_readme_convergence(world, root)
# === VIVARIUM + CONVERGENCE RENDER LAYER v2.0.1 END ===
